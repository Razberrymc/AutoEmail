...
for index, row in prop.iterrows():
    ...
    if ((row['Exemptions_Flag']) == 1) and (Field1Value != (row['Exemptions1']) and (str(row['Exemptions1']).strip() == "nan")):
        """
        masterExemptionList = []
        primExemptionListNoSpaces = []

        primExemptionList = str(row['Exemptions2']).strip().split(",")

        for element in primExemptionList:
            primExemptionListNoSpaces.append(element.strip())

        exemptionCodes=pd.read_csv(r'\Users\mclark\Desktop\python\email\exemption_codes_and_descriptions.csv')

        coreExemptionCodes=exemptionCodes["exemption_cd"]
        coreExemptionList = coreExemptionCodes.to_list()

        coreExemptionDescription=exemptionCodes["exemption_desc"]
        coreDescriptionList = coreExemptionDescription.to_list()

        def list_match(primExemptionListNoSpaces, coreExemptionList):
            for prim in primExemptionListNoSpaces:
                for core in coreExemptionList:
                    if prim == core:
                        masterExemptionList.append(coreDescriptionList[coreExemptionList.index(core)])
                        #print(masterExemptionList)

        list_match(primExemptionListNoSpaces, coreExemptionList)
        """
        ...
        primExemptionList = str(row['Exemptions1']).strip().split(",")
        ...
        masterExemptionList2 = []
        primExemptionListNoSpaces2 = []

        primExemptionList2 = str(row['Exemptions2']).strip().split(",")

        for element in primExemptionList2:
            primExemptionListNoSpaces2.append(element.strip())

        exemptionCodes2=pd.read_csv(r'\Users\mclark\Desktop\python\email\exemption_codes_and_descriptions.csv')

        coreExemptionCodes2=exemptionCodes2["exemption_cd"]
        coreExemptionList2 = coreExemptionCodes2.to_list()

        coreExemptionDescription2=exemptionCodes2["exemption_desc"]
        coreDescriptionList2 = coreExemptionDescription2.to_list()

        def list_match(primExemptionListNoSpaces2, coreExemptionList2):
            for prim2 in primExemptionListNoSpaces2:
                for core2 in coreExemptionList2:
                    if prim2 == core2:
                        masterExemptionList2.append(coreDescriptionList2[coreExemptionList2.index(core2)])

        list_match(primExemptionListNoSpaces2, coreExemptionList2)
        """
    ...
    if write_email == True:
        ...
        if Field1Value == str(row['Exemptions1']).strip():
            Field1Value = str(masterExemptionList)
        if Field2Value == (row['Exemptions2']).strip():
            Field2Value = str(masterExemptionList2)
        """
